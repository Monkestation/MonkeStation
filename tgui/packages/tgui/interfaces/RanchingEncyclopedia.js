...
import { Flex, Icon, Box, Section, Tabs, Stack } from "../components";
import { Window } from "../layouts";

export const RanchingEncyclopedia = (props, context) => {
  const { act, data } = useBackend(context);

  const {
    chicken_list = [],
  } = data;
  const [selectedChicken] = useLocalState(context, "chicken", chicken_list[0]);
...
const ChickenInfo = (props, context) => {
  ...
  return (
    <Flex class="chicken-info-container">
      ...
      <Flex.Item class="chicken-metric">
        {selectedChicken.required_species && "A " + selectedChicken.required_species + " needs to be present for this chicken's birth."}
      </Flex.Item>
      <Flex.Item class="chicken-metric">
        {selectedChicken.player_health && "A crew member that has been injured by atleast "+ selectedChicken.player_health + " points."}
      </Flex.Item>
      <Flex.Item class="chicken-metric">
        {selectedChicken.required_atmos && "Chicken needs to be an environment with: " + selectedChicken.required_atmos + " present."}
      </Flex.Item>
      <Flex.Item class="chicken-metric">
        {selectedChicken.required_rooster && "A " + selectedChicken.required_rooster + " needs to be present for this chicken's birth."}
      </Flex.Item>
      <Flex.Item class="chicken-metric">
        {selectedChicken.liquid_depth && "Their needs to be a pool of liquid atleast" + selectedChicken.liquid_depth +" deeo."}
      </Flex.Item>
      <Flex.Item class="chicken-metric">
...
const ChickenTabs = (props, context) => {
  const { data } = useBackend(context);
