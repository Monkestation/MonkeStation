...
/datum/component/singularity
	...
	/// Callback for admin_investigate
	var/datum/callback/admin_investigate_callback
...
/datum/component/singularity/Initialize(
	...
	if(notify_admins) //no reason to create a callback datum if we do not use it
...
/datum/component/singularity/proc/admin_investigate_setup()
	...
	var/atom/atom_parent = parent
	atom_parent.investigate_log("was made a singularity at [AREACOORD(spawned_turf)].", INVESTIGATE_ENGINES)
...
/datum/component/singularity/proc/bluespace_reaction()
	...
	var/atom/atom_parent = parent
	atom_parent.investigate_log("has been shot by bluespace artillery and destroyed.", INVESTIGATE_ENGINES)
	qdel(parent)
