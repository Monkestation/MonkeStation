...
/datum/component/footstep/proc/play_footstep()
	...
	if(!T.footstep || LM.buckled || LM.lying || !CHECK_MULTIPLE_BITFIELDS(LM.mobility_flags, MOBILITY_STAND | MOBILITY_MOVE) || LM.throwing || LM.movement_type & (VENTCRAWLING | FLYING))
		if (LM.lying && !LM.buckled && !(!T.footstep || LM.movement_type & (VENTCRAWLING | FLYING))) //play crawling sound if we're lying
			playsound(T, 'sound/effects/footstep/crawl1.ogg', 15 * v, mixer_channel = CHANNEL_SOUND_FOOTSTEPS)
		return
	...
	//for slimes
	if(isslime(LM))
		playsound(T, 'sound/effects/footstep/slime1.ogg', 15 * v, mixer_channel = CHANNEL_SOUND_FOOTSTEPS)
		return
	...
	if(isshoefoot(LM))
		...
		if(ishuman(LM)) //for proper humans, they're special
			...
			if(H.dna.species.special_step_sounds)
				playsound(T, pick(H.dna.species.special_step_sounds), 50, TRUE, mixer_channel = CHANNEL_SOUND_FOOTSTEPS)
