...
/datum/strippable_item/mob_item_slot/start_equip(atom/source, obj/item/equipping, mob/user)
	...
	if(!ismob(source))
		return FALSE
	if(!do_mob(user, source, get_equip_delay(equipping), show_to_world = TRUE, add_image = equipping.appearance))
		return FALSE
...
/proc/start_unequip_mob(obj/item/item, mob/source, mob/user, strip_delay)
	if(!do_mob(user, source, strip_delay || item.strip_delay, show_to_world = TRUE, add_image = item.appearance))
		return FALSE
