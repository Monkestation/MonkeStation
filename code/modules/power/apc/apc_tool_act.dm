...
/obj/machinery/power/apc/crowbar_act(mob/user, obj/item/crowbar)
	...
	if(opened && integration_cog)
		user.visible_message(span_notice("[user.name] begins prying something out of the APC."))
		balloon_alert(user, "prying something out of the APC")
		crowbar.play_tool_sound(src)
		if(crowbar.use_tool(src, user, 50))
			user.visible_message(span_warning("[user.name] screws up breaking whatever was inside!"))
			balloon_alert(user, "screwed up removing what was inside")
...
/obj/machinery/power/apc/screwdriver_act(mob/living/user, obj/item/screwdriver)
	if(..())
		return TRUE
	. = TRUE
...
/obj/machinery/power/apc/wirecutter_act(mob/living/user, obj/item/wirecutter)
	if (terminal && opened)
		terminal.dismantle(user, wirecutter)
		return TRUE
