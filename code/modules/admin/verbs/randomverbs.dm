...
/client/proc/respawn_character()
	...
	if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
		...
		var/id = md5(RUSTG_HASH_MD5, "[G_found.real_name][G_found.mind.assigned_role]")
...
/client/proc/smite(mob/living/target as mob)
	...
	switch(punishment)
		...
		//monkestation edit begin: weregorilla
		if(ADMIN_PUNISHMENT_WEREGORILLA)
			if(target.mind)
				target.mind.AddSpell(new /obj/effect/proc_holder/spell/targeted/shapeshift/weregorilla)
			else
