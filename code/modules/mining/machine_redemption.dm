...
/obj/machinery/mineral/ore_redemption/Initialize(mapload)
	...
	local_storage = AddComponent(/datum/component/material_container, ALL_MATERIALS, INFINITY, FALSE, null, null, allowed_types = /obj/item/stack)
...
/obj/machinery/mineral/ore_redemption/RefreshParts()
	...
	var/sheet_per_ore_temp = 3 //Triple default
	for(var/obj/item/stock_parts/matter_bin/B in component_parts)
		sheet_per_ore_temp = 2 + B.rating //3, 4, 5, 6
	for(var/obj/item/stock_parts/micro_laser/L in component_parts)
		point_upgrade_temp = 0.65 + (0.35 * L.rating)
	point_upgrade = point_upgrade_temp
	sheets_per_ore = sheet_per_ore_temp

/obj/machinery/mineral/ore_redemption/examine(mob/user)
	. = ..()
	if(in_range(user, src) || isobserver(user))
		. += "<span class='notice'>The status display reads: Smelting <b>[sheets_per_ore]</b> sheet(s) per piece of ore.<br>Reward point generation at <b>[point_upgrade*100]%</b>.</span>"
	if(panel_open)
...
/obj/machinery/mineral/ore_redemption/proc/smelt_ore(obj/item/stack/ore/O)
	...
	if(!material_amount)
		...
		if(!local_storage.has_space(material_amount * sheets_per_ore * O.amount) && O?.refined_type) //if there is no space, eject it
			var/obj/item/stack/sheet/overflow = new O.refined_type
			overflow.amount = (O.amount * sheets_per_ore)
			unload_mineral(overflow)
		else
			local_storage.insert_item(O, sheets_per_ore) //insert it
		qdel(O)
