...
/mob/living/simple_animal/bot/attackby(obj/item/W, mob/user, params)
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(!(bot_cover_flags & BOT_COVER_LOCKED))
			...
			to_chat(user, "<span class='notice'>The maintenance panel is now [bot_cover_flags & BOT_COVER_OPEN ? "opened" : "closed"].</span>")
		else
		...
		if(bot_cover_flags & BOT_COVER_OPEN)
			...
			if(do_after(user, 30, target = src))
				if (paicard)
					user.visible_message("<span class='notice'>[user] uses [W] to pull [paicard] out of [initial(src.name)]!</span>","<span class='notice'>You pull [paicard] out of [initial(src.name)] with [W].</span>")
					ejectpai(user)
	else
...
/mob/living/simple_animal/bot/proc/scan(list/scan_types, old_target, scan_range = DEFAULT_SCAN_RANGE)
	...
	if(!current_turf)
		return
	var/list/adjacent = current_turf.GetAtmosAdjacentTurfs(1)
...
/mob/living/simple_animal/bot/ui_act(action, params)
	...
	if(action == "lock")
		bot_cover_flags ^= BOT_COVER_LOCKED
	if(bot_cover_flags & BOT_COVER_LOCKED && !(issilicon(usr) || IsAdminGhost(usr)))
		return
	switch(action)
		if("power")
			if(bot_mode_flags & BOT_MODE_ON)
