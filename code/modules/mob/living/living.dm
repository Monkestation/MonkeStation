...
/mob/living/Bump(atom/A)
	...
	if(buckled || now_pushing)
		return
	if((confused || is_blind()) && stat == CONSCIOUS && (mobility_flags & MOBILITY_STAND) && m_intent == "run" && (!ismovableatom(A) || is_blocked_turf(A)) && !HAS_MOB_PROPERTY(src, PROP_CANTBUMPSLAM) &&!has_status_effect(SUGAR_RUSH) && !has_status_effect(HEN_RUSH))  // ported from VORE, sue me
