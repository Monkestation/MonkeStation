...
/datum/species/proc/handle_body(mob/living/carbon/human/human_host)
	...
	if(HD && !(HAS_TRAIT(human_host, TRAIT_HUSK)))
		...
		if(ALTEYESPRITES in species_traits)
			...
			var/mutable_appearance/eye_overlay
			if(!E)
				eye_overlay = mutable_appearance(human_host.dna.species.alt_eye, "eyes_missing", -BODY_LAYER)
			else
				eye_overlay = mutable_appearance(human_host.dna.species.alt_eye, E.eye_icon_state, -BODY_LAYER)
			if((EYECOLOR in species_traits) && E)
				eye_overlay.color = "#" + human_host.eye_color
			standing += eye_overlay
		//monkestation edit end
			...
			if(!(HAS_TRAIT(human_host, TRAIT_HUSK)))
				if(!forced_colour)
					switch(accessory_type.color_src)
						if(SKINTONES)
							accessory_overlay.color = "#[GLOB.skin_tones[human_host.dna.species.skin_tone_list][human_host.skin_tone]]"
						if(MUTCOLORS)
							if(fixed_mut_color)
						...
						if(CUSTOM)
							if(!human_host.custom_color) //if they don't have one, we just return random.  TODO: make sprite accessories and bodyparts inherit their owners colors.
								human_host.custom_color = random_short_color()
							accessory_overlay.color = "#[human_host.custom_color]"
						//monkestation edit end
				else
	...
	switch(slot)
		...
		if(ITEM_SLOT_SUITSTORE)
			...
			if(I.w_class > WEIGHT_CLASS_BULKY)
				...
				return FALSE
			if( istype(I, /obj/item/pda) || istype(I, /obj/item/pen) || is_type_in_list(I, human_host.wear_suit.allowed) )
				return TRUE
			return FALSE
...
/datum/species/proc/handle_digestion(mob/living/carbon/human/human_host)
	...
	//metabolism change
	if(human_host.nutrition > NUTRITION_LEVEL_FAT && !human_host.current_hungerlevel_flag == NUTRITION_LEVEL_FAT)
	...
	else if(human_host.nutrition < NUTRITION_LEVEL_FAT && human_host.nutrition > NUTRITION_LEVEL_FED && human_host.satiety > 80 && !(human_host.current_hungerlevel_flag == NUTRITION_LEVEL_FED) && !HAS_TRAIT(human_host, TRAIT_NOHUNGER))
...
/datum/species/proc/spec_emag_act(mob/living/carbon/human/human_host, mob/user)
	return
