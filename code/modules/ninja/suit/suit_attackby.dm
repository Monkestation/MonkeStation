/obj/item/clothing/suit/space/space_ninja/attackby(obj/item/I, mob/ninja, params)
	...
	if(istype(I, /obj/item/reagent_containers/glass) && I.reagents.has_reagent(/datum/reagent/uranium/radium, suit_radium_refill) && adrenaline_available != TRUE)//If it's a glass beaker, and what we're transferring is radium.
		I.reagents.remove_reagent(/datum/reagent/uranium/radium, suit_radium_refill)
		adrenaline_available = TRUE;
		...
		if(CELL.maxcharge > cell.maxcharge && n_gloves?.candrain)
			...
			if (n_gloves?.candrain && do_after(ninja,suit_action_delay, target = src))
