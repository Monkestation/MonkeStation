...
/obj/anomaly/singularity/spatial_rift
...
/obj/anomaly/singularity/spatial_rift/Initialize(mapload)
	. = ..()
	QDEL_IN(src, 5 SECONDS) // vanishes after 5 seconds

/obj/anomaly/singularity/spatial_rift/process()
	consume()

/obj/anomaly/singularity/spatial_rift/consume(atom/A)
	if(isturf(A))
		A.singularity_act()
		return
...
/obj/anomaly/singularity/spatial_rift/admin_investigate_setup()
	var/turf/T = get_turf(src)
...
/obj/anomaly/singularity/spatial_rift/attack_tk(mob/living/user)
	if(!istype(user))
		return
