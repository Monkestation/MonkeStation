...
/obj/machinery/computer/piratepad_control/multitool_act(mob/living/user, obj/item/multitool/I)
	if(istype(I) && istype(I.buffer,/obj/machinery/piratepad/civilian))
		to_chat(user, "<span class='warning'>You link [src] with [I.buffer] in [I] buffer.")
...
/obj/machinery/computer/piratepad_control/civilian/send()
	playsound(loc, 'monkestation/sound/machines/wewewew.ogg', 70, TRUE)
	if(!sending)
		return
	if(!inserted_scan_id)
		stop_sending()
	...
	pad.visible_message("<span class='notice'>[pad] activates!")
...
/obj/machinery/computer/piratepad_control/civilian/proc/id_eject(mob/user, obj/target)
	if(!target)
		to_chat(user, "<span class='warning'>That slot is empty!")
		return FALSE
	else
...
/obj/item/bounty_cube/examine()
	. = ..()
	if(speed_bonus)
		. += "<span class='notice'><b>[time2text(next_nag_time - world.time,"mm:ss")]</b> remains until <b>[bounty_value * speed_bonus]</b> credit speedy delivery bonus lost."
	if(handler_tip && !bounty_handler_account)
		. += "<span class='notice'>Scan this in the cargo shuttle with an export scanner to register your bank account for the <b>[bounty_value * handler_tip]</b> credit handling tip."
...
/obj/item/bounty_cube/debug_cube/attack_self(mob/user)
	if(!isliving(user))
		to_chat(user, "<span class='warning'>You aren't eligible to use this!")
		return ..()

	if(!set_up)
		var/mob/living/carbon/human/squeezer = user
		if(squeezer.get_bank_account())
			set_up(random_bounty(), squeezer.get_idcard())
			set_up = TRUE
			return ..()
		to_chat(user, "<span class='warning'>It can't detect your bank account.")
...
/obj/item/civ_bounty_beacon/attack_self()
	loc.visible_message("<span class='warning'>\The [src] begins to beep loudly!")
