...
/datum/station_trait/new_space_law
	...
	weight = 10
	show_in_report = TRUE
	report_message = "A recent bet at centcom has led to a new space law being created. A report on it will be printed at all security record consoles."
...
/datum/station_trait/new_space_law/on_round_start()
	...
	for(var/obj/machinery/computer/secure_data/console in GLOB.machines)
		if(!(console.machine_stat & (BROKEN|NOPOWER)) && is_station_level(console.z))
			var/obj/item/paper/lawprint = new /obj/item/paper(console.loc)
			lawprint.name = "paper - 'New Space Law'"
			lawprint.info = "Due to recent events in space politics [new_space_law] now 1xx illegal under space law."
			lawprint.update_icon()
